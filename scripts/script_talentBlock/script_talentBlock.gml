...
//hp, lastHp, status, blockColor, maxBlock, blockMeter, decayRate, rechargeRate, shieldBroken

//Grant block and gradually reduce block meter, won't allow if shield is broken
if (mouse_check_button(mb_right) and blockMeter > 0 and !shieldBroken) { //if holding right click
...
//Shield break, if block meter reaches 0, must wait until shield is full to use again
if (blockMeter == 0) {
	shieldBroken = true;
}
if (blockMeter == maxBlock) { //resets broken status at max block meter
	shieldBroken = false;
}
